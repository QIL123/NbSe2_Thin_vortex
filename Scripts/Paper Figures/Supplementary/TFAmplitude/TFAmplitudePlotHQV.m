...
mar=0.13; %margin
%Titles
Bz=text(figs(1,1),0,Y_size+1.2*mar,'B_z(h,  )','Units','in','Color',[0 0 0],'FontSize',12,'FontName',FontName);
Bzrbold=text(figs(1,1),0.4,Y_size+1.5*mar,'r','Units','in','Color',[0 0 0],'FontSize',12,'FontName',FontName,'FontWeight','bold');
dbzdx=text(figs(1,2),0,Y_size+1.2*mar,'d{B_z}(h,  )/dx','Units','in','Color',[0 0 0],'FontSize',12,'FontName',FontName);
dbzdxrbold=text(figs(1,2),0.49,Y_size+1.5*mar,'r','Units','in','Color',[0 0 0],'FontSize',12,'FontName',FontName,'FontWeight','bold');
Bac=text(figs(1,3),0,Y_size+1.2*mar,'{{B^{ac}_z}}(h,  )/{x_{ac}}','Units','in','Color',[0 0 0],'FontSize',12,'FontName',FontName);
Bacrbold=text(figs(1,3),0.465,Y_size+1.3*mar,'r','Units','in','Color',[0 0 0],'FontSize',12,'FontName',FontName,'FontWeight','bold');
...
%% ploting and saving only surfaces separatly
% Figure - Fits and deferent images


newfig1=figure('NumberTitle', 'off', 'Name', 'Fit');
Position1=[0.75 0.75 width len];
clf
...
for i=1:Xnum
    posfig(1,i+3)={[X0+spaceX*i+i*X_size-0.3*X_size Y0+spaceY*1.1+Y_size*1 X_size*0.3 Y_size*0.05]};
    figs(1,i+3)=axes('Parent',newfig1,...
        'ZColor',fontcolor,'YColor',fontcolor,'XColor',fontcolor,...
        'LineWidth',linewidth,...
        'FontSize',fontsize,...
        'FontName',FontName,...
