...
Path1=sprintf('C:\\Users\\Owner\\Dropbox\\QIL projects\\NbSe2_thin vortex\\Nofar\\Nofar 5 layer sample\\drive-download-20230518T094026Z-001\\%s\\SXM\\NbS00882%s.sxm',Day,num1);
Path2=sprintf('C:\\Users\\Owner\\Dropbox\\QIL projects\\NbSe2_thin vortex\\Nofar\\Nofar 5 layer sample\\drive-download-20230518T094026Z-001\\%s\\SXM\\NbS00882%s.sxm',Day,num2);

sxmFile1 = sxm.load.loadProcessedSxM(Path1);
sxmFile2 = sxm.load.loadProcessedSxM(Path2);

header=sxmFile1.header;

Scan_Pixels=sxmFile1.header.scan_pixels;

Scan_Range=sxmFile1.header.scan_range;
...
DataACY1=sxmFile1.channels(TF_y_channel).rawData; %tunning fork channel Y backward
DataACY2=sxmFile2.channels(TF_y_channel).rawData; %tunning fork channel Y backward
DataTFy=DataACY1-DataACY2;


DataACX1=sxmFile1.channels(TF_x_channel).rawData; %tunning fork channel Y backward
DataACX2=sxmFile2.channels(TF_x_channel).rawData; %tunning fork channel Y backward
DataTFx=DataACX1-DataACX2;


DataDC1=sxmFile1.channels(DC_channel).rawData; %tunning fork channel Y backward
DataDC2=sxmFile2.channels(DC_channel).rawData; %tunning fork channel Y backward
DataDC=DataDC1-DataDC2;
...
sgtitle(['Pearl Length Fit Data Diff = ',num2str(PearlLengthTFDataDifferentiation),'Pearl Length Fit model Diff = ',num2str(PearlLengthTFModelDerivative),' Dist = ',num2str(DistanceFromSampleDataDifferentiation),num2str(DistanceFromSampleModelDerivative)...
    ,' Tip Response = ',num2str(TipResponseDataDifferentiation),' Tip Response = ',num2str(TipResponseModelDerivative),' number of layers=',num2str(LayerN),' day ',Day,' nums of file name ending',num1,num2])
...
sgtitle(['Pearl Length Fit Data Diff = ',num2str(PearlLengthTFDataDifferentiation),'Pearl Length Fit model Diff = ',num2str(PearlLengthTFModelDerivative),' DistImageDiff = ',num2str(DistanceFromSampleModelDerivative),num2str(DistanceFromSampleModelDerivative)...
    ,' Tip Response = ',num2str(TipResponseDataDifferentiation),' Tip Response = ',num2str(TipResponseModelDerivative),' number of layers=',num2str(LayerN),' day ',Day,' nums of file name ending',num1,num2])