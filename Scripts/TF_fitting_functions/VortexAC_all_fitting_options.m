...
figure(1515)
subplot(1,5,1)
surf(X,Y,MTFDataDifferentiation)% plotting data
view(2);
shading flat 
hold on
...
for i=1:length(xEx)
    plot3(xEx(i)+R*cos(t),yEx(i)+R*sin(t),R*cos(t)*0+abs(max(max(MTFDataDifferentiation)))*1.1,'k')
    plot3(xEx(i)+R2*cos(t),yEx(i)+R2*sin(t),R2*cos(t)*0+abs(max(max(MTFDataDifferentiation)))*1.1,'k')
    
    plot3(xEx(i)+r*cos(t),yEx(i)+r*sin(t),r*cos(t)*0+abs(max(max(MTFDataDifferentiation)))*1.1,'r')
    
    text(xEx(i),yEx(i),abs(max(max(MTFDataDifferentiation))),num2str(i))
    %plot3(xEx(i),yEx(i),abs(max(max(M))),'.','markersize',10,'color',[1 1 1]*0)
end
%plotting the error between the model and the data
Y1=Y;
X1=X;
Y1(end+1,:)=((Y(2,1)-Y(1,1))+Y(end,1))+X(1,:)*0;
Y1(:,end+1)=Y1(:,1);
X1(:,end+1)=((X(1,2)-X(1,1))+X(1,end))+X(1,:)*0;
X1(end+1,:)=X1(1,:);

[BzDataDifferentiation,ConvBzDataDifferentiation] = pearlgen_no_mag(X1,Y1,xc,yc,PearlLengthTFDataDifferentiation,DistanceFromSampleDataDifferentiation,phi_0,Kernel);
...
surf(X,Y,MTFDataDifferentiation-ConvBzDataDifferentiation_diff)
view(2);
shading flat;
caxis([min(min(MTFDataDifferentiation)) max(max(MTFDataDifferentiation))])
axis square
...
surf(X,Y,ConvBzDataDifferentiation_diff)
view(2);
shading flat;
...
subplot(1,5,4)
% surf(X(aa-l:aa+l,bb-l:bb+l),Y(aa-l:aa+l,bb-l:bb+l),M(aa-l:aa+l,bb-l:bb+l)-Bz(aa-l:aa+l,bb-l:bb+l))
surf(X,Y,MTFModelDerivative-ConvBz_diff_model_derivative)
view(2);
shading flat;
caxis([min(min(MTFModelDerivative)) max(max(MTFModelDerivative))])
axis square
...
subplot(1,5,5)
% surf(X(aa-l:aa+l,bb-l:bb+l),Y(aa-l:aa+l,bb-l:bb+l),Bz(aa-l:aa+l,bb-l:bb+l))
surf(X,Y,ConvBz_diff_model_derivative)
view(2);
shading flat;
...
figure(555)
subplot(6,6,[1 2 7 8])
surf(X,Y,MTFDataDifferentiation)
shading interp
title('Data')
axis equal
...
caxis([min(MTFDataDifferentiation(:)),max(MTFDataDifferentiation(:))])
axis equal
...
caxis([min(MTFModelDerivative(:)),max(MTFModelDerivative(:))])
axis equal
...
[~,bb]=find(X==xc,1);
plot(X(1,:),MTFDataDifferentiation(:,bb))
xlabel('y [\mum]')
ylabel('B [G]')
grid on
axis([X(1,1) X(1,end) min(MTFDataDifferentiation(:)),max(MTFDataDifferentiation(:))+(max(MTFDataDifferentiation(:))/10)])
hold on
plot(X(1,:),ConvBzDataDifferentiation_diff(:,bb))
hold off
legend('Data','Model image diff')
subplot(6,6,[32 33])
title('Cross-Section image diff fit')
[aa,~]=find(Y==yc,1);
plot(Y(:,1),MTFDataDifferentiation(aa,:))
xlabel('x [\mum]')
ylabel('B [G]')
grid on
axis([Y(1,1) Y(end,1) min(MTFDataDifferentiation(:)),max(MTFDataDifferentiation(:))+(max(MTFDataDifferentiation(:))/10)])
hold on
plot(Y(:,1),ConvBzDataDifferentiation_diff(aa,:))
legend('Data','Model image diff')
hold off
subplot(6,6,[28 29])
title('Cross-Section analitical derivative')
plot(X(1,:),MTFModelDerivative(:,bb))
xlabel('y [\mum]')
ylabel('B [G]')
grid on
axis([X(1,1) X(1,end) min(MTFModelDerivative(:)),max(MTFModelDerivative(:))+(max(MTFModelDerivative(:))/10)])
hold on
plot(X(1,:),ConvBz_diff_model_derivative(:,bb))
hold off
legend('Data','Model analitical derivative')
subplot(6,6,[34 35])
title('Cross-Section analitical derivative')
plot(Y(:,1),MTFModelDerivative(aa,:))
xlabel('x [\mum]')
ylabel('B [G]')
grid on
axis([Y(1,1) Y(end,1) min(MTFModelDerivative(:)),max(MTFModelDerivative(:))+(max(MTFModelDerivative(:))/10)])
hold on
plot(Y(:,1),ConvBz_diff_model_derivative(aa,:))
legend('Data','Model analitical derivative')
hold off
